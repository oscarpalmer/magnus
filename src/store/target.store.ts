...
import type {TargetsCallbacks} from '../models';

export class Targets {
	private readonly callbacks: TargetsCallbacks | undefined;

	private readonly store = new Map<string, Set<Element>>();

	get readonly(): TargetsCallbacks {
		return this.callbacks as TargetsCallbacks;
	}
	...
	find(selector: string): Element | null {
		return typeof selector === 'string' ? this.context.state.element.querySelector(selector) : null;
	}

	findAll(selector: string): Element[] {
		return typeof selector === 'string'
			? [...this.context.state.element.querySelectorAll(selector)]
			: [];
	}
